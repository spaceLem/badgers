function figS1(gen=0, nvals = [26, 26, 5])
	if !exist("~/badger_data/figS1-summary.mat", "file") || gen == 12345
		if !exist("~/badger_data/figS1-data.mat", "file") || gen == 12345
...
function gen_data(nvals)
	...
	vals{3} = [0.5 0.75 1.0 1.25 1.5];    % r

	% create fresh file, removing old one if necessary
	save -7 ~/badger_data/figS1-data.mat P par nvals vals

	for k = 1:nvals(3); for j = 1:nvals(2); for i = 1:nvals(1)
		P.p1 = vals{1}(i);
		P.p2 = vals{2}(j);
		eval(sprintf("P.%s = %i;", par{3}, vals{3}(k))); % P.r = vals{3}(k);
		P.name = sprintf("dS1_%i_%i_%i", i, j, k); % di_j_k

		generate(P);

		load(P.name)
		save("-append", "-7", "~/badger_data/figS1-data.mat", P.name)
		delete(P.name)
...
function summarise_data()
	load ~/badger_data/figS1-data.mat % P par nvals vals
	...
	for k = 1:nvals(3)
		data = eval(sprintf("dS1_0_%i.data", k));

		Pi0 = mean(data, 3)(2, tidx);
		G0  = mean(data, 3)(4, tidx);

		for j = 1:nvals(2); for i = 1:nvals(1)
			data = eval(sprintf("dS1_%i_%i_%i.data", i, j, k));
	...
	save -7 ~/badger_data/figS1-summary.mat G N Pi P par nvals vals Gi Ni Zi xi yi Xi Yi X Y
endfunction


function plot_data()
	load ~/badger_data/figS1-summary.mat %  G N Pi P par nvals vals Gi Ni Zi xi yi Xi Yi X Y
